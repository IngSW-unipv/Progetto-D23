...
public class ConfermaPrenotazionePazienteActionListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			ArrayList<SlotCalendarioSingoli> slot_liberi = model.getArraySlotLiberi();
			int indice = model.getIndexArraySlotLiberi();
			SlotCalendarioSingoli slotCorrente = (SlotCalendarioSingoli) slot_liberi.listIterator(indice);
			...
			// id prenotazione
			model.setUltimaPrenotazione();
			int lastId = model.getUltimaPrenotazione();
			int idNewPren = lastId + 1;

			// mi serve anche la PrestazioneSanitaria
			LocalTime durata = LocalTime.of(0, 30, 0);
			PrestazioneSanitaria prestSan = new PrestazioneSanitaria(tipoPrest, durata);

			// genero la prenotazione
			Prenotazione newPren = new Prenotazione(idNewPren, paz, personaleSanitario, prestSan, data, orario);

			model.inserisciPrenotazione(newPren);
